		...
		if ( results.Tables["Results"].Rows.Count > 0 ) {
			...
			foreach ( DataRow r in results.Tables["Results"].Rows ) {
				...
				//__ Generate, Save O-Form ___________________________________
					...
					this.EfxLib.libAutoRpt.saveReport(
						orderNum, orderLine, ofName, ofReport, ofImage );
				...
				if ( (int)r["Calculated_MaxOL"]==orderLine ) { 
					...
					//__ Generate, Save O-Form ___________________________________
						...
						// Call saveReport Function
						this.EfxLib.libAutoRpt.saveReport(
							orderNum, orderLine, soaName, "SOAck_New", "" );
					...
					//__ Create, send email w/ attachments _______________________
						// Old SmtpClient and MailMessage classes, necessary to use different Smtp than the one in company configuration
						...
						// Send email
						smtp.Send(message);
							addLog(string.Format("Order {0} Email sent to {1} with {2} attachments.", orderNum, dest, AttachmentFiles.Count));
