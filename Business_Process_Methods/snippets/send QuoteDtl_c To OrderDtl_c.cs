//In-Event Data Directive to Copy all UD Field Values from QuoteDtl to OrderDtl;
...
var qd = Db.OrderDtl.Where(x => 
			   x.OrderNum  == 7520098 
			&& x.OrderLine == 1)
		.FirstOrDefault();


string[] columns = qd.First().GetType().GetProperties();

StringBuilder sQd = new StringBuilder();
Action<string> go = s => { sQd.Append(s).AppendLine(); };

Action<string> colName = c => { go((string) Row[c]); };

qd.First().GetType().GetProperties().Select(p => p.Name).ToList().ForEach(colName);


//go(qd[Inputs.cReinforceType_c.Value].ToString());


Inputs.eInfo.Value = sQd.ToString();
...
qd.First().GetType().GetProperties().Select(p => p.Name).ToList().ForEach(processColumnName);
...
foreach ( var od in ttOrderDtl.Where(x => x.RowMod=="A" && x.QuoteNum!=0) ) {
	...
	DataTable qd = Db.QuoteDtl.Where(x => x.Company == CompanyID
		&& x.QuoteNum  == od.QuoteNum
		&& x.QuoteLine == od.QuoteLine)
	.FirstOrDefault();
	...
	if ( od != null && qd != null ) {
		...
		foreach ( DataColumn column in qd.Columns ) {
			if ( column.ColumnName.EndsWith("_c") ) od[column.ColumnName] = qd[column.ColumnName];
		}
	}
}


foreach ( var qd_c in (Db.QuoteDtl.Where(x => x.QuoteNum == od.QuoteNum && x.QuoteLine == od.QuoteLine).Select(s => s))[0] ) {

}



var od = Db.OrderDtl.Where(x => x.OrderNum == 7522735 && x.OrderLine = 1).Select(s => s).ToList();

StringBuilder tbl = new StringBuilder();

Action<string> go = s => { tbl.Append(s).AppendLine(); };
...
Inputs.eInfo.Value = tbl.ToString();
...
IEnumerable<IDictionary<string, object>> GetValues<T>(DataContext context, IQueryable<T> query) where T : class {
    ...
    foreach(var entity in context.GetTable<T>()) {
        ...
        yield return (context.Mapping.GetTable(typeof(T)).RowType.DataMembers
			...
			.Select(name => new { name, value = propertyInfos[name].GetValue(entity, null) })
...
foreach ( var od in ttOrderDtl.Where(x => x.RowMod == "A" && x.QuoteNum != 0) ) {
	...
	if ( od != null && qd != null ) {
		...
		Action<string> udField = col => {
			if ( col.EndsWith("_c") ) od[col] = qd[col];
		}

		qd.GetType().GetProperties().Select(s => s.Name).ToList().ForEach(udField);
	}
}
...
qd.GetType().GetProperties().Select(p => p.Name).ToList().ForEach(colName);
